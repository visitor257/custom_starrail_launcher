...
fn="custom_starrail_launcher_v1.0"
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.settingsel_change2i_downloadsel = QComboBox(self.settings_group)
        self.settingsel_change2i_downloadsel.addItem("")
        self.settingsel_change2i_downloadsel.addItem("")
        self.settingsel_change2i_downloadsel.addItem("")
        self.settingsel_change2i_downloadsel.setObjectName(u"downloadsel")
        self.settingsel_change2i_downloadsel.setGeometry(QRect(415, 420, 181, 31))
        self.settingsel_change2i_downloadsel.setFont(font2)
        self.settingsel_change2i_downloadsel.setMaxVisibleItems(4)
        self.settingsel_change2i_downloadsel.setInsertPolicy(QComboBox.InsertAtBottom)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.settingsel_change2i_downloadsel.setItemText(0, QCoreApplication.translate("MainWindow", u"\u767e\u5ea6\u7f51\u76d8", None))
        self.settingsel_change2i_downloadsel.setItemText(1, QCoreApplication.translate("MainWindow", u"123\u7f51\u76d8", None))
        self.settingsel_change2i_downloadsel.setItemText(2, QCoreApplication.translate("MainWindow", u"Google Drive", None))
...
class mainwin(QMainWindow,Ui_MainWindow):
    ...
    def main_uiitemsetup(self):
        ...
        self.uigroup_daily_open=[self.modesel,self.mode_set,self.start_game,self.start_launcher]
        ...
        self.uigroup_settings_change2i=[self.settingsel_change2i_zip,self.settingsel_change2i_zip_selbt,self.settingsel_change2i_zip_show,self.settingsel_change2i_selbt,self.settingsel_change2i_download,self.settingsel_change2i_article,self.settingsel_change2i_delete,self.settingsel_change2i_downloadsel]
        ...
        self.gl_setup_text.setPixmap(QPixmap("custom_launcher_sources\\sl_setup_text.png"))
        ...
        self.bwiki_web.setUrl("https://wiki.biligame.com/sr/%E9%A6%96%E9%A1%B5")
    ...
    def ui_setup(self):
        ...
        self.mode_dict={1:["1","mihoyo","1",""],2:["1","mihoyo","1",""],3:["14","bilibili","0",""],4:["1","hoyoverse_PC","1",""]}
        self.mode_dict_det={1:[["1","gw_PC","2",""],["1","mihoyo","1",""]],
                            2:[["1","gw_PC","1",""],["1","mihoyo","1",""]],
                            3:[["14","bilibili","0",""]],
                            4:[["1","hoyoverse_PC","1",""]]}
        try:
    ...
    def sel_gs(self):
        ...
        if sel_file and sel_file != "":
            ...
            self.settingsel_change2i_zip_show.setText("未选择")
            sel_file = path.split(sel_file)[0]
            with open(sel_file+"/config.ini","r") as f:
                current_cps=f.read()
            current_cps=current_cps.split("cps=")[-1].split("\n")[0]
            if not current_cps=="hoyoverse_PC":
                if "PCGameSDK.dll" not in sel_file+"/StarRail_Data/Plugins":
                    copy("custom_launcher_sources\\run.dll",f"{sel_file}\\StarRail_Data\\Plugins\\PCGameSDK.dll")
            else:
                if "PCGameSDK.dll.backupc" not in sel_file+"/StarRail_Data/Plugins":
                    copy("custom_launcher_sources\\run.dll",f"{sel_file}\\StarRail_Data\\Plugins\\PCGameSDK.dll.backupc")
    ...
    def get_index(self, lst):
        b = None
        for i in self.mode_dict_det:
            for j in self.mode_dict_det[i]:
                if lst == j:
                    b = i
        return b
    ...
    def modesel_event_2(self,g1,g2,org_id):
        ...
        with open(self.config_content_dict["game_downloaded_path"]+"/config.ini","r") as f:
            current_cps=f.read()
        current_cps=current_cps.split("cps=")[-1].split("\n")[0]
        dec_result=current_cps=="hoyoverse_PC"
        ...
        if tar_s==org_s:
            ...
            if (current_cps=="hoyoverse_PC")==dec_result:
                self.start_launcher.setVisible(True)
                self.start_launcher.setEnabled(True)
            ...
            for i in itg:
                i.setVisible(False)
                i.setEnabled(False)
            if path.exists("custom_launcher_sources/icchange"):
                self.start_game.setVisible(True)
                self.start_game.setEnabled(True)
            else:
    ...
    def c_s(self,tar_idx):
        ...
        if now_s==tar_s:
            ...
            if tar_s=="i":
                    ...
                    if i!="":
                        try:
                            if self.sources_path_in_zip!="":
                                ...
                                isp=i
                            if isp==zip_f+".exe":
                                rename(gsp+"/StarRail.exe",gsp+"/StarRail.exe.backupc")
                                rename(gsp+"/StarRail.exe.backupi",gsp+"/StarRail.exe")
                ...
                if fnc==False:
                    rename(gsp+"/"+"StarRail_Data",gsp+"/"+"StarRail_Data")
            else:
                    ...
                    if i!="":
                        try:
                            if self.sources_path_in_zip!="":
                                ...
                                isp=i
                            if isp == zip_f + ".exe":
                                rename(gsp + "/StarRail.exe", gsp + "/StarRail.exe.backupi")
                                rename(gsp + "/StarRail.exe.backupc", gsp + "/StarRail.exe")
                            elif isp[-13:] == "PCGameSDK.dll":
                                pass
                            else:
            ...
            if button == Qt.MouseButton.LeftButton:
                ...
                if self.eventtype_setting==False:
                    ...
                    if self.detect_pos(x,y,self.settingsel_change2i_zip_selbt,self.settings_group)==True and self.settingsel_change2i_zip_selbt.isVisible():
                        self.sel_zip()
                    if self.detect_pos(x,y,self.settingsel_change2i_article,self.settings_group)==True and self.settingsel_change2i_article.isVisible():
                        self.start_url("https://www.bilibili.com/video/BV1rT411872K/")
                    if self.detect_pos(x,y,self.settingsel_change2i_download,self.settings_group)==True and self.settingsel_change2i_download.isVisible():
                        with open("custom_launcher_sources/source_download.txt") as f:
                            dLinks=f.read()
                        dLinks=dLinks.split("\n")
                        self.start_url(dLinks[self.settingsel_change2i_downloadsel.currentIndex()])
        else:
            if button==Qt.MouseButton.LeftButton:
                self.eventtype_leftmouse = False
                x = event.pos().x()
                ...
                if 0 <= y <= 41 and x<1172:
                    ...
                    QMessageBox().warning(self,"注意","崩坏：星穹铁道 已启动，为避免程序或文件错误，该功能已禁用，以及请勿关闭启动器")
    ...
    def detect_launched_1(self):
        if self.proc_exist("StarRail.exe")==True or self.proc_exist("StarRail.exe")==True:
